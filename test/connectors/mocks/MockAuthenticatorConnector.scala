...
trait MockAuthenticatorConnector extends UnitTestTrait with MockHttp {
  ...
  def setupMockMatchClient(clientDetailsModel: Option[UserDetailsModel] = None)(status: Int, response: JsValue): Unit =
    setupMockHttpPost(TestAuthenticatorConnector.matchingEndpoint, clientDetailsModel)(status, response)
  ...
  val setupMockMatchClient: UserDetailsModel => ((Int, JsValue)) => Unit =
    (clientDetailsModel: UserDetailsModel) => (setupMockMatchClient(None) _).tupled
  ...
  val matchUserMatched: (Int, JsValue) = (OK,
    """{
      | "firstName" : "",
      | "lastName" : "",
      | "dateOfBirth" : "",
